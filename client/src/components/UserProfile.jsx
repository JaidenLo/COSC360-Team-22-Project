...
function UserProfile({ userId, username, email, usertype, city, aboutMe }) {
    ...
    if (file.size > 2 * 1024 * 1024) {
        setUploadError('Image is too large. Please choose an image under 2MB.');
        e.target.value = '';
        return;
    }
    setUploadError('');
            ...
            <div className="profile-top">
                ...
                <div className="profile-info">
                    <h2 className="profile-name">
                        {username}
                        {usertype === 'admin' && <span className="admin-tag" onClick={() => {navigate("/admin-dashboard")}}>Dashboard</span> } 
                    </h2>
